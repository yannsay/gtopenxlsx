...
write_stub_and_table_body <- function(gt_table, ordered_gt_data, wb, sheet_name, row_to_start){
  ...
  if(rlang::is_empty(gt_table$`_row_groups`)) {
    ...
    openxlsx::writeData(wb, sheet_name, ordered_gt_data, startRow = row_to_start, colNames = F,
              borders = "surrounding",
              borderColour ="#D3D3D3")
    ...
    for(i in unique(gt_table$`_row_groups`)) {
      ...
      openxlsx::writeData(wb, sheet_name, data_to_write, startRow = row_start_stub, colNames = F,
                borders = "surrounding",
                borderColour ="#D3D3D3")
